...
\midrule
100 & 0.004 & 1.22 $\pm$ 0.33 & 30.39 $\pm$ 3.82 \\
1,000 & 0.049 & 4.06 $\pm$ 1.74 & 35.38 $\pm$ 6.27 \\
10,000 & 0.605 & 9.73 $\pm$ 2.58 & 41.81 $\pm$ 4.66 \\
100,000 & 7.146 & 65.94 $\pm$ 15.89 & 126.25 $\pm$ 24.85 \\
500,000 & 39.566 & 496.47 $\pm$ 328.79 & 702.64 $\pm$ 223.93 \\
\botrule
\end{tabular}
\end{table}
...
\midrule
100 & 0.004 & 4.30 $\pm$ 2.11 & 6.39 $\pm$ 4.11 \\
1,000 & 0.049 & 8.73 $\pm$ 3.09 & 13.43 $\pm$ 8.33 \\
10,000 & 0.605 & 25.24 $\pm$ 6.47 & 33.40 $\pm$ 10.98 \\
100,000 & 7.146 & 154.66 $\pm$ 34.00 & 204.52 $\pm$ 42.73 \\
500,000 & 39.566 & 888.52 $\pm$ 317.44 & 1,300.45 $\pm$ 373.84 \\
\botrule
\end{tabular}
\end{table}

A save through IPFS took 29 to 32~ms longer than a save to an XMI file for models up to 10,000 elements. The difference therefore stays almost constant while the model grows, which points to a fixed cost per call to the local Kubo node. In relative terms, a save through IPFS took about 25 times the XMI time at 100 elements, 8.7 times at 1,000 elements and 4.3 times at 10,000 elements. For the two largest models, the difference grows to 60~ms and 206~ms, while the ratio falls to 1.9 and 1.4. Loading from IPFS took 1.3 to 1.5 times the time of loading the XMI file at every size.

A Wilcoxon signed-rank test compares the thirty paired iterations of each model size, because both paths run inside the same iteration on the same content. The test gives $p < 0.001$ for every model size and for both operations, so the differences are unlikely to come from noise. Nevertheless, single iterations vary. The standard deviation of the XMI save reaches 328.79~ms at 500,000 elements, and the median of 390.32~ms stays well below the mean of 496.47~ms, so a few slow iterations carry the mean. The full distribution of every measurement is in the result file of the repository.
...
For RQ3, a save through a local Kubo node added 29 to 32~ms per model for models up to 10,000 elements, and 60~ms to 206~ms for the two largest models. The relative cost falls from about 25 times the XMI time at 100 elements to 1.4 times at 500,000 elements, because the added time comes mostly from a fixed cost per call. Loading from IPFS took 1.3 to 1.5 times the time of loading a local XMI file. The measured costs appear acceptable for interactive editing, where a save of a model with 10,000 elements stays under 50~ms. Nevertheless, remote nodes and IPNS resolution still need measurement.
...
The main threat to internal validity comes from the shared Java virtual machine, because garbage collection can affect single iterations. The standard deviations at 500,000 elements show the effect. The benchmark reduces the risk with three warm-up iterations, thirty measured iterations per model size and a paired statistical test. The cascade tests also cover small resources only.
...
With a local Kubo node, a save through IPFS added 29 to 32~ms per model for models up to 10,000 elements, and the relative cost fell from about 25 times the XMI time at 100 elements to 1.4 times at 500,000 elements. The MoDisco case study published 1.04~GiB of Java models as 6,933 resources. EMF Compare found no differences for eight components with 1,639 resources.
