...
%% and Notation (BPMN)"; the abstract defines its own abbreviations.
...
\item IPFS support in the Eclipse BPMN2 Modeler for Business Process Model and Notation (BPMN) diagrams and in the generated BPMN2 tree editor (Section~\ref{sec:tool}).
...
Content addressing identifies data by a hash of the data. Merkle introduced hash trees for authentication~\cite{merkle1988signature}. Venti used a hash of the block contents as the identifier of a block in archival storage~\cite{quinlan2002venti}. Git stores every object under the Secure Hash Algorithm 1 (SHA-1) checksum of the object content~\cite{chacon2014progit}.
...
Figure~\ref{fig:architecture} shows the main classes of the plugin as a Unified Modeling Language (UML) class diagram. \texttt{IPFSResourceImpl} extends the XMI resource of EMF, and \texttt{IPFSResourceFactoryImpl} creates the IPFS resources. \texttt{IPFSURIHandlerImpl} extends the default URI handler of EMF. \texttt{IPFSModelPersister} provides the cascade save (Section~\ref{sec:rq2-cascade}), and \texttt{ReferenceMode} selects references by CID or by IPNS name.
...
\caption{Unified Modeling Language (UML) class diagram of the ecorefs core plugin. Classes marked ecorefs extend classes and interfaces of the EMF persistence API. The Kubo node offers the HTTP API for saving and loading.}\label{fig:architecture}
\end{figure*}
...
Figure~\ref{fig:save-load} shows the save and load interactions as a UML sequence diagram. Saving an \texttt{IPFSResourceImpl} takes three steps. First, the resource writes the XMI serialization to a temporary file. The temporary file avoids keeping a large serialized model in memory. Second, the file is added to the local Kubo node, and Kubo returns the CID. Third, the resource URI is replaced with \texttt{ipfs://} followed by the CID.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/save-load.pdf}
\caption{UML sequence diagram of saving a resource to IPFS and loading the resource again. During loading, the URI converter of the resource set selects \texttt{IPFSURIHandlerImpl} for \texttt{ipfs://} and \texttt{ipns://} URIs.}\label{fig:save-load}
\end{figure*}
...
\begin{figure}[tbp]
\centering
\includegraphics{figures/version-pinning.pdf}
...
\begin{figure}[tbp]
\centering
\includegraphics{figures/publication-layers.pdf}
...
\begin{figure}[tbp]
\centering
\includegraphics{figures/rq3-performance.pdf}
...
\begin{figure}[tbp]
\centering
\includegraphics{figures/compare-scaling.pdf}
