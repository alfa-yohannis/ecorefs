...
\midrule
100 & 0.004 & 0.60 $\pm$ 0.25 & 29.86 $\pm$ 1.56 \\
1,000 & 0.049 & 0.91 $\pm$ 0.29 & 28.73 $\pm$ 1.50 \\
10,000 & 0.605 & 3.54 $\pm$ 0.54 & 32.45 $\pm$ 2.21 \\
100,000 & 7.146 & 33.05 $\pm$ 2.47 & 79.12 $\pm$ 4.44 \\
500,000 & 39.566 & 353.84 $\pm$ 40.01 & 518.20 $\pm$ 137.97 \\
\botrule
\end{tabular}
\end{table}
...
\midrule
100 & 0.004 & 1.88 $\pm$ 0.81 & 3.10 $\pm$ 3.25 \\
1,000 & 0.049 & 2.41 $\pm$ 0.74 & 2.84 $\pm$ 0.76 \\
10,000 & 0.605 & 8.46 $\pm$ 0.77 & 10.92 $\pm$ 3.03 \\
100,000 & 7.146 & 77.07 $\pm$ 4.74 & 95.91 $\pm$ 6.60 \\
500,000 & 39.566 & 602.99 $\pm$ 57.24 & 830.18 $\pm$ 85.07 \\
\botrule
\end{tabular}
\end{table}

A save through IPFS took 28 to 29~ms longer than a save to an XMI file for models up to 10,000 elements. The difference therefore stays almost constant while the model grows, which points to a fixed cost per call to the local Kubo node. In relative terms, a save through IPFS took about 49 times the XMI time at 100 elements, 32 times at 1,000 elements and 9.2 times at 10,000 elements. For the two largest models, the difference grows to 46~ms and 164~ms, while the ratio falls to 2.4 and 1.5. Loading from IPFS took 1.2 to 1.7 times the time of loading the XMI file.

A Wilcoxon signed-rank test compares the thirty paired iterations of each model size, because both paths run inside the same iteration on the same content. The test gives $p < 0.01$ for every model size and for both operations, so the differences are unlikely to come from noise. The standard deviation stays below 12\% of the mean in most series. Two series vary more. The loads of the smallest models vary because the times are close to one millisecond. The IPFS save at 500,000 elements reaches a deviation of 137.97~ms through a single iteration of 1,211~ms, while the median is 489~ms and the interquartile range is 53~ms. A save of that size writes 39.6~MB of new blocks into the store of the node, so a flush inside the node lands in one iteration. Without that iteration, the mean is 494~ms and the deviation is 44~ms. The full distribution of every measurement is in the result file of the repository.
...
For RQ3, a save through a local Kubo node added 28 to 29~ms per model for models up to 10,000 elements, and 46~ms to 164~ms for the two largest models. The relative cost falls from about 49 times the XMI time at 100 elements to 1.5 times at 500,000 elements, because the added time comes mostly from a fixed cost per call. Loading from IPFS took 1.2 to 1.7 times the time of loading a local XMI file. The measured costs appear acceptable for interactive editing, where a save of a model with 10,000 elements stays under 35~ms. Nevertheless, remote nodes and IPNS resolution still need measurement.
...
The main threat to internal validity comes from the shared Java virtual machine, because garbage collection can affect single iterations. The benchmark reduces the risk with a fixed heap of 8~GB, a collection between the iterations, three warm-up iterations, thirty measured iterations per model size and a paired statistical test. An earlier run on a busy machine gave times up to four times higher, so the reported run used an otherwise idle machine. The cascade tests also cover small resources only.
...
With a local Kubo node, a save through IPFS added 28 to 29~ms per model for models up to 10,000 elements, and the relative cost fell from about 49 times the XMI time at 100 elements to 1.5 times at 500,000 elements. The MoDisco case study published 1.04~GiB of Java models as 6,933 resources. EMF Compare found no differences for eight components with 1,639 resources.
