...
The cascade test builds two resources with one object each. The object in resource A has a non-containment reference to the object in resource B. The test calls \texttt{cascadeSave} on B and measures the time for both saves. Over ten runs, the cascade save took 47~ms on average, with a median of 47~ms and a standard deviation of 2.77~ms. The measurement covers two IPFS saves of very small resources, so the time comes almost completely from the two calls to the local node. However, the test does not measure the search for cross-references separately.

Three further tests check the rewritten references. Each test builds resources with one node, saves every resource, changes the last resource and calls \texttt{cascadeSave}. After the cascade, each test fetches the stored bytes of every referencing resource from the node and checks that the reference carries the current CID of the target. The first test uses a chain of two resources. The second test uses the diamond of Figure~\ref{fig:cascade}, where the collaboration refers to the order process and to the payment process. The third test uses a chain of four resources, where the cascade save took 120~ms for four saves. All three tests pass.
