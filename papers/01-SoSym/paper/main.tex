...
\caption{Unified Modeling Language (UML) class diagram of the ecorefs core plugin. Each class marked ecorefs extends one class of the EMF persistence API. The IPFS node runs Kubo and offers the HTTP API for saving and loading. Each row of the first two rows holds one class of EMF with the ecorefs class that extends the class.}\label{fig:architecture}
\end{figure*}
...
The approach loads each resource as a whole. Database-backed persistence can load parts of a model on demand~\cite{pagan2011morsa,daniel2017neoemf}, but the IPFS layer cannot. The approach also offers no query support comparable to Object Constraint Language (OCL) queries inside a database~\cite{omg2014ocl,daniel2016mogwai}. Furthermore, IPNS resolution needs an extra lookup and can take longer than direct CID access. A node that takes part in the public network also announces every new block to the distributed hash table, which adds work to each save. In the runs of this paper, the fixed cost per save was about 21~ms with an offline node and about 29~ms with a node in the public network. The announcement carries no model data, because another node receives a copy only after a request for the CID. In CID mode, a change to a leaf resource causes saves along the whole dependency chain. Cyclic references always leave one outdated CID. Published models stay available only while at least one node stores the content.
...
The paper studied EMF model persistence on IPFS along three research questions.

RQ1 asked for the architectural challenges of content-addressed persistence and for a way to address the challenges. The challenges come from the changing identity of a resource, because a resource has no identifier before the first save, every save produces a new identifier, and content under an existing identifier cannot be overwritten. Three extension points of EMF answer the challenges: a resource that saves to IPFS and takes the new CID as the resource URI, a factory for the \texttt{ipfs} and \texttt{ipns} schemes, and a read-only URI handler for loading. Placeholder URIs cover the time before the first save. File resources and IPFS resources share one resource set, and a reference across two resources on IPFS resolves through the standard proxy mechanism.

RQ2 asked how cross-resource references survive in an environment where identifiers change with every modification. A reference by CID pins the exact version of the target. An upgrade therefore needs new saves along the referencing resources, and the cascade save orders those saves by the dependencies between the resources. Tests on a chain, on a diamond and on a chain of four resources confirm that every rewritten reference carries the current identifier, and the BPMN case study shows the same result for three real BPMN files. References by IPNS name avoid the cascade at the price of immutability. For a partitioned model, component directories, manifests and one IPNS name give a publication structure with a single mutable head.

RQ3 asked for the performance overhead and for the conditions of practical use. With a local Kubo node, a save through IPFS added 20 to 22~ms per model for models up to 10,000 elements, and the relative cost fell from about 31 times the XMI time at 100 elements to 1.4 times at 500,000 elements. A load from IPFS took 1.1 to 1.4 times the time of a local file read. The cost therefore suits interactive editing on a local node, while a model that grows beyond a few hundred thousand elements pays mostly for the serialization and not for the storage layer. The MoDisco case study published 1.04~GiB of Java models as 6,933 resources, and EMF Compare found no differences for eight components with 1,639 resources.

Future work follows the open parts of the same three questions. For RQ1, the challenges specific to EMF have to be separated from the challenges of other modeling frameworks. For RQ2, the cascade save needs a run on a large dependency graph, for example inside the partitioned MoDisco model. For RQ3, remote nodes, IPNS resolution, a shared network file system as a further baseline and a comparison with database-backed persistence remain open.
