...
%% Times New Roman for the text and matching mathematics
...
\toprule
Duration & 6 months \\
Target venue & \href{https://link.springer.com/journal/10270}{Software and Systems Modeling (SoSyM)}, with an earlier conference paper \\
...
Figure~\ref{fig:settings} contrasts a local development setting for a model that is split into fragments with the decentralized setting that the study targets. In the local setting, every party reads and writes one shared location, and each reference names a path. In the decentralized setting, every fragment becomes an immutable block, each reference names the CID of the target, and a mutable IPNS name marks the current version.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{../../../papers/01-SoSym/paper/figures/development-settings.pdf}
\caption{Development of a model that is split into fragments. (a) In a local setting, the fragments sit in one shared location, and each reference names a path. (b) In the decentralized setting of the study, each fragment is an immutable block on IPFS, each reference names a CID, and a mutable IPNS name marks the current version of the project.}\label{fig:settings}
\end{figure}
...
Package & Work & Months \\
\midrule
WP1 & Problem analysis and literature review, with a verified reference list & 1 \\
WP2 & Persistence layer for \texttt{ipfs://} and \texttt{ipns://} (RQ1) & 1 to 3 \\
WP3 & Reference strategies, the cascade update and the publication structure (RQ2) & 2 to 4 \\
WP4 & Tool support in the Eclipse BPMN2 Modeler and in the generated tree editor & 3 to 4 \\
WP5 & Evaluation: benchmark, functional tests and the case study (RQ3) & 4 to 5 \\
WP6 & Writing, artifact release and submission & 5 to 6 \\
\bottomrule
\end{tabular}
\end{center}

Figure~\ref{fig:workplan} shows the schedule. Milestones follow the packages. M1 at month 1 is the problem analysis with the reference list. M2 at month 3 is a persistence layer that saves and loads a model through a local node. M3 at month 4 is a working update of referencing resources with tests, together with the tool support inside an editor. M4 at month 5 is the complete evaluation data. M5 at month 6 is the submitted manuscript with a public artifact.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figures/work-plan.pdf}
\caption{Schedule of the work packages over six months, with the five milestones.}\label{fig:workplan}
\end{figure}
...
\begin{longtable}{@{}p{32mm}p{58mm}rr@{}}
\toprule
...
\midrule
\endhead
Researcher & Honorarium, six months at IDR 2,000,000 per month & 706 & 12,000,000 \\
Assistant & Part time, six months at IDR 2,000,000 per month & 706 & 12,000,000 \\
Storage and pinning & Retention of the published dataset & 250 & 4,250,000 \\
Domain names & Two names at Cloudflare for the tunnels, where the service itself costs nothing & 22 & 374,000 \\
Archiving & Deposit under a digital object identifier, free on Zenodo & 0 & 0 \\
...
Meals and transport & Five days & 250 & 4,250,000 \\
\midrule
Subtotal & Without the open access fee & 3,494 & 59,394,000 \\
Publication & Open access fee of the journal, optional & 3,190 & 54,230,000 \\
